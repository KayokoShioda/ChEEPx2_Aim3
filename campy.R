...
# for Campylobacter that has 5 pathways (person-to-person, water, food, 
...
# Packages
library(lhs)
library(deSolve) # For ODE (deterministic model)
library(ggplot2) 
library(SobolSequence)

# Differential equations
SIWFCOS_model <- function(t,       # Time 
  ...
  Iho <- x[6] # Number of individuals infected by other sources
  W   <- x[7] # Pathogens in water
  F   <- x[8] # Pathogens in food
  C   <- x[9] # Prevalence among live chickens
  O   <- x[10] # Pathogens in "other:
  N   <- S + (Ihh + Ihw + Ihf + Ihc + Iho) 
  
  # Define each rate moving from one compartment to the other
  gamma  <- parms[6] # Recovery rate 
  alpha  <- parms[7] # Shedding rate into water
  xi     <- parms[8] # Clearance rate from water
  mu     <- parms[9] # Birth and death rate
  
  # Scale transmission rates by gamma+mu 
  beta_I <- parms[1]*(gamma+mu)/(Ihh+Ihw+Ihf+Ihc+Iho) # Transmission rate from person to person
  ...
  beta_O <- parms[5]*(gamma+mu)/O # Transmission rate from live chicken to person 
  
  dxdt <- numeric(length(x))
  dxdt[1] <- mu*N -beta_I*S*(Ihh+Ihw+Ihf+Ihc+Iho) - beta_W*S*W - beta_F*S*F - beta_C*S*C - beta_O*S*O - mu*S + gamma*(Ihh+Ihw+Ihf+Ihc+Iho) # dS/dt
  dxdt[2] <-       beta_I*S*(Ihh+Ihw+Ihf+Ihc+Iho) - gamma*Ihh - mu*Ihh # dIhh/dt (person-to-person transmission)
  ...
  dxdt[6] <-       beta_O*S*O                     - gamma*Iho - mu*Iho # dIho/dt (all other-to-person transmission) <-- NEW!!
  dxdt[7] <-       alpha*(Ihh+Ihw+Ihf+Ihc+Iho) - xi*W # dW/dt (Pathogen shedding into water and clearance from water)
  dxdt[8] <- 0 # dF/dt 
  dxdt[9] <- 0 # dC/dt
  dxdt[10]<- 0 # dO/dt
...
numparam <- 6 # 6 for the Campylobacter model and 7 for the Salmonella model
...
lhs_high <-  c(0.2, rep(1, numparam-1))
...
set.x0        <- c(S[1], Ihh[1], Ihw[1], Ihf[1], Ihc[1],  Iho[1], W[1], F[1], C[1], O[1])
names(set.x0) <- c('S', 'Ihh',  'Ihw',  'Ihf',  'Ihc',   'Iho',  'W',  'F',  'C',  'O') 
...
env.conc <- numeric(numsamples) # pathogen concentration in the environmental source (water in this case) in the endemic status
...
ode_func <- function (lambda_gm, # prevalence (all pathways) scaled by gamma*mu 
  ...
  out <- ode(y = set.x0,              # initial state values
             ...
             func   = SIWFCOS_model,  # ODE function
  ...
  out$I <- out$Ihh + out$Ihw + out$Ihf + out$Ihc + out$Iho
  ...
  NLL = -sum(759*c(0.08*c(0.04,0.09,0.57,0.17,0.13), 1-0.08) * log(c(prev_Ihh, prev_Ihw, prev_Ihf, prev_Ihc, prev_Iho, 1-prev_I)))
  
  # Output
  return(c(NLL, prev_I, prev_Ihh, prev_Ihw, prev_Ihf, prev_Ihc, prev_Iho, W_end))
}
...
for(i in 1:numsamples){
  ...
  parsample[i,] <- rho[i,]  # scaled Sobol sampled parameter sets (prevalence and AFs)
  res.prev[i,]  <- temp[2:(numparam+1)] # estimated prevalence (prev_I, prev_Ihh, prev_Ihw, prev_Ihf, prev_Ihc, prev_Iho)
  env.conc[i]  <- temp[numparam+2]
}

parsample <- as.data.frame(parsample)
names(parsample) <- c("baseline.prev", "AF_h", "AF_w", "AF_f", "AF_c", "AF_o")
res.prev <- as.data.frame(res.prev)
names(res.prev) <- c("prev_I", "prev_Ihh", "prev_Ihw", "prev_Ihf", "prev_Ihc", "prev_Iho")
